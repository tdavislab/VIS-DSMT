class DMT3{
    ...
    saveConfig(){
        ...
        console.log()
        let secondLine = vlength.toString()+" "+flength.toString()+" "+elength.toString()+"\n";
        config += secondLine;
        for(let vId in this.vertices){
            let v = this.vertices[vId];
            let line = v.xcoord + " " + v.ycoord + " " + v.value +
             "\n";
            config += line;
        }
        for(let fId in this.faces){
            let f = this.faces[fId];
            let line = "";
            f.point.forEach(v=>{
                line += v.id + " ";
            })
        ...
        for(let eId in this.edges){
            ...
            let line = e.start.id + " " + e.end.id + " " + e.value + "\n";
            config += line;
